...
const ProductCard = ({ product, onAddToCart }: ProductCardProps) => (
    ...
    >
        ...
        <div className="p-3 flex flex-col flex-grow">
            <h3 className="font-bold text-slate-800 text-sm flex-grow">{product.name}</h3>
...
const ProductListItem = ({ product, onAddToCart }: ProductCardProps) => (
    ...
    >
        ...
        <div className="flex-grow overflow-hidden pr-2">
            <p className="font-bold text-slate-800 text-sm truncate" title={product.name}>{product.name}</p>
...
const ProductGrid = ({ products, onAddToCart }: ProductGridProps) => {
    ...
    const handleSearchKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
        if (e.key === 'Enter') {
            // Attempt to find a product by SKU when enter is pressed in the search box
            const foundProduct = products.find(p => p.sku.trim().toLowerCase() === searchTerm.trim().toLowerCase());
            if (foundProduct) {
