...
const NavButton = ({ view, label, currentView, onClick, children }: NavButtonProps) => {
    ...
    return (
        <motion.button
            ...
            className={`relative flex flex-col items-center justify-center w-full py-3 px-2 rounded-xl transition-all duration-200 group ${
                isActive
                    ? 'bg-emerald-50 text-emerald-600 dark:bg-slate-700 dark:text-white'
                    : 'text-slate-500 hover:bg-emerald-50 hover:text-emerald-600 dark:text-slate-400 dark:hover:bg-slate-700 dark:hover:text-slate-200'
            }`}
        ...
        >
            ...
            {/* Hover Indicator: A subtle bar that appears on hover for inactive items */}
            {!isActive && (
                <div className="absolute left-0 top-1/4 bottom-1/4 w-1 bg-emerald-300 rounded-r-full opacity-0 group-hover:opacity-100 transition-opacity duration-200" />
            )}
...
const SidebarContent = ({ currentView, setCurrentView, role, permissions }: Omit<SidebarProps, 'isOpen' | 'setIsOpen'>) => {
    ...
    return (
        <div className="w-24 bg-slate-50 dark:bg-slate-800 flex flex-col items-center p-2 shadow-2xl z-20 h-full">
            <div className="flex items-center justify-center h-16 w-full flex-shrink-0">
                <div className="bg-gradient-to-br from-emerald-500 to-emerald-600 text-white text-xl font-bold rounded-xl w-12 h-12 flex items-center justify-center shadow-md">
                    K
                </div>
            </div>
